...
class ActorNetwork(object):
    ...
    of all actions.
    """
    def __init__(self, sess, state_dim, action_dim, learning_rate, entropy_weight=ENTROPY_WEIGHT,
    ...
    def create_actor_network(self):
        with tf.compat.v1.variable_scope('actor'):
            ...
            split_6 = tflearn.conv_1d(inputs[:, 6:7, :self.future_step], 128, 4, activation='relu')
            ...
            merge_net = tflearn.merge([split_0, split_1, split_2_flat, split_3_flat, split_4_flat, split_5, split_6_flat], 'concat')
...
class CriticNetwork(object):
    ...
    """
    def __init__(self, sess, state_dim, learning_rate, future_step = FUTURE_STEP):
        self.sess = sess
        self.s_dim = state_dim
        self.lr_rate = learning_rate
        self.future_step = future_step
...
class PenaltyNetwork(object):
    # similar to the critic network, but predict the expected total stall times
    # staring at state s and following the policy pi.
    def __init__(self, sess, state_dim, learning_rate, future_step = FUTURE_STEP):
        self.sess = sess
        self.s_dim = state_dim
        self.lr_rate = learning_rate
        ...
        # Create the penalty network
        self.inputs, self.out = self.create_penalty_network()

        # Get all network parameters
        self.network_params = \
            tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.TRAINABLE_VARIABLES, scope='penalty')
        ...
        # Network target U(s)
        self.stall_time_target = tf.compat.v1.placeholder(tf.float32, [None, 1])

        # Stall Time Difference, will also be weights for actor_gradients
        self.std = tf.subtract(self.stall_time_target, self.out)

        # Mean square error
        self.loss = tflearn.mean_square(self.stall_time_target, self.out)

        # Compute penalty gradient
        self.penalty_gradients = tf.gradients(self.loss, self.network_params)

        # Optimization Op
        self.optimize = tf.compat.v1.train.RMSPropOptimizer(self.lr_rate). \
            apply_gradients(zip(self.penalty_gradients, self.network_params))

    def create_penalty_network(self):
    ...
    def train(self, inputs, stall_time_target):
        return self.sess.run([self.loss, self.optimize], feed_dict={
            self.inputs: inputs,
            self.stall_time_target: stall_time_target
        })
    ...
    def get_std(self, inputs, stall_time_target):
        return self.sess.run(self.std, feed_dict={
            self.inputs: inputs,
            self.stall_time_target: stall_time_target
        })

    def get_gradients(self, inputs, stall_time_target):
        return self.sess.run(self.penalty_gradients, feed_dict={
            self.inputs: inputs,
            self.stall_time_target: stall_time_target
        })

    def apply_gradients(self, penalty_gradients):
        return self.sess.run(self.optimize, feed_dict={
            i: d for i, d in zip(self.penalty_gradients, penalty_gradients)
        })
...
def compute_gradients_with_penalty(s_batch, a_batch, r_batch, stall_batch, terminal, actor, critic, penalty):
    """
    batch of s, a, r is from samples in a sequence
    the format is in np.array([batch_size, s/a/r_dim])
    terminal is True when sequence ends as a terminal state
    ...
    v_batch = critic.predict(s_batch)
    u_batch = penalty.predict(s_batch)

    R_batch = np.zeros(r_batch.shape)
    U_batch = np.zeros(r_batch.shape)
    ...
    if terminal:
        ...
        R_batch[-1, 0] = v_batch[-1, 0]  # boot strap from last state
        U_batch[-1, 0] = u_batch[-1, 0]

    for t in reversed(range(ba_size - 1)):
        R_batch[t, 0] = r_batch[t] + GAMMA * R_batch[t + 1, 0]  # td target
        U_batch[t, 0] = stall_batch[t] + U_batch[t + 1, 0]

    td_batch = R_batch - v_batch  # advantage function

    actor_gradients = actor.get_gradients(s_batch, a_batch, td_batch)
    critic_gradients = critic.get_gradients(s_batch, R_batch)
    penalty_gradients = penalty.get_gradients(s_batch, U_batch)

    return actor_gradients, critic_gradients, td_batch, penalty_gradients, u_batch
